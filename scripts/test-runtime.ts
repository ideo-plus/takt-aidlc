...
export const codexTestRuntime = resolve(repo, '.experiments/cache/aidlc-v2.8.2/dist/codex');

export async function prepareTestRuntime(host: 'claude' | 'codex' = 'claude') {
  const runtime = host === 'codex' ? codexTestRuntime : testRuntime;
  if (!runtime.startsWith(join(repo, '.experiments/cache') + '/')) throw new Error('テスト用runtimeは.experiments/cache内に配置してください');
  ...
  const marker = join(runtime, `.${host}/tools/aidlc-version.ts`);
  if (!existsSync(marker)) {
    mkdirSync(runtime, { recursive: true });
    requireSuccess(await command(['aidlc', 'config', '--harness', host, '--yes'], runtime, cleanEnvironment(), 60000));
  }
