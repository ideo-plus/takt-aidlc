...
test('移動した配布物からConstructionの品質ゲートとレビューを実行できる', async () => {
  ...
  expect(existsSync(join(moved, 'takt/workflows/aidlc-construction.yaml'))).toBe(true);
...
test('CG設定はInception承認で起動せず、CG入口から一度だけ自動実行する', async () => {
  ...
  expect(existsSync(join(moved, 'takt/workflows/aidlc-code-generation.yaml'))).toBe(true);
