...
import { hash, sources } from "./cg-gate";
...
export function stageGate(control: string, phase: string) {
  const ctx = read(join(control, "stage-context.json")),
    root = ctx.workspace;
  ...
  const draftPath = join(reports, "01-stage.json"),
    reviewPath = join(reports, "02-stage-review.json");
  const ledgerPath = join(control, "stage-ledger.json"),
  ...
  const artifactDir = join(root, "input/stage-output");
