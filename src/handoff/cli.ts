...
try {
  ...
  if (['session', 'hook', 'plugin-hook', 'codex-session', 'codex-hook'].includes(mode) && cgEnabled(project)) {
  ...
  if (mode === 'session' || mode === 'codex-session') {
    ...
    throw new Error('usage: bun handoff.js session|hooks|hook|plugin-hook|codex-session|codex-hook|cg-work|cg-status|work|retry|status <project> [id]');
  }
} catch (error) {
