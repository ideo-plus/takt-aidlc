...
export function cgGate(control: string, phase: string) {
  ...
  const checkPlan = () => {
    ...
    assert.equal(r.planHash, reportHash('01-cg-plan.json'), 'レビュー後に計画が変化');
    assert.equal(r.reportHash, reportHash('02-plan-review.json'), '計画レビューが変化');
  ...
  const checkTraceability = () => {
    ...
    const plan = report('01-cg-plan.json');
  ...
  const checkReviewed = () => {
    ...
    assert.equal(r.reportHash, reportHash('03-code-review.json'));
  ...
  if (phase === 'result') {
    ...
    for (const name of ['01-cg-plan.json', '02-plan-review.json', '03-code-review.json']) {
    ...
    assert.equal(latest('finish')?.reportHash, reportHash('04-cg-summary.json'));
  ...
  const blockerReport: Record<string, string> = { plan: '01-cg-plan.json', 'plan-review': '02-plan-review.json', 'code-review': '03-code-review.json' };
  ...
  if (phase === 'plan') {
    ...
    const p = report('01-cg-plan.json');
    ...
    binding = { planHash: reportHash('01-cg-plan.json'), planMarkdownHash: hash(planProjection(readFileSync(join(root, 'cg/code-generation-plan.md'), 'utf8'))), instructionsHash: hash(readFileSync(join(root, 'cg/unit-test-instructions.md'))) }; verdict = 'ready';
  } else if (phase === 'plan-review' || phase === 'code-review') {
    if (phase === 'plan-review') { assert.deepEqual(current, ctx.initialSources); assert.equal(latest('plan')?.planHash, reportHash('01-cg-plan.json')); }
    else { checkPlan(); checkTests(); checkTraceability(); }
    const name = phase === 'plan-review' ? '02-plan-review.json' : '03-code-review.json';
    const r = report(name);
    ...
    verdict = r.verdict; binding = { planHash: reportHash('01-cg-plan.json'), reportHash: reportHash(name) };
    ...
    checkReviewed(); const r = report('04-cg-summary.json');
    ...
    binding = { reportHash: reportHash('04-cg-summary.json'), summaryHash: hash(readFileSync(join(root, 'cg/code-summary.md'))) };
